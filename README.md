<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a0033,50:4b0082,100:6a0dad&height=220&section=header&text=Rashmini%20Kavindya&fontSize=52&fontColor=e0d4ff&animation=fadeIn&fontAlignY=38&desc=Full%20Stack%20Developer%20%7C%20UI%2FUX%20Enthusiast%20%7C%20IT%20Undergraduate&descAlignY=58&descSize=18" width="100%"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=800&color=A78BFA&center=true&vCenter=true&width=600&lines=Full+Stack+Developer+(MERN+%2B+Java);UI%2FUX+Designer+%40+Figma;3rd+Year+IT+Undergraduate+%40+SLIIT;Building+Real-World+Web+%26+Mobile+Solutions" alt="Typing SVG" />

<br/>

<img src="https://img.shields.io/badge/BSc%20(Hons)%20ICT-Specialized%20in%20IT-6a0dad?style=flat-square&labelColor=1a0033"/>
<img src="https://img.shields.io/badge/SLIIT-3rd%20Year%20Undergraduate-4b0082?style=flat-square&labelColor=1a0033"/>
<img src="https://img.shields.io/badge/HND%20in%20IT-Completed-8a2be2?style=flat-square&labelColor=1a0033"/>
<img src="https://img.shields.io/badge/📍-Attidiya,%20Sri%20Lanka-6a0dad?style=flat-square&labelColor=1a0033"/>

<br/><br/>

<a href="https://rashmini-kavindya.vercel.app"><img src="https://img.shields.io/badge/Portfolio-Visit%20Site-8a2be2?style=for-the-badge&logo=vercel&logoColor=white&labelColor=1a0033"/></a>
<a href="https://www.linkedin.com/in/rashmini-k"><img src="https://img.shields.io/badge/LinkedIn-Connect-4b0082?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=1a0033"/></a>
<a href="mailto:rashminik727@gmail.com"><img src="https://img.shields.io/badge/Email-Contact%20Me-6a0dad?style=for-the-badge&logo=gmail&logoColor=white&labelColor=1a0033"/></a>
<a href="https://github.com/rashmini-k"><img src="https://img.shields.io/badge/GitHub-Follow-a78bfa?style=for-the-badge&logo=github&logoColor=white&labelColor=1a0033"/></a>

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=rashmini-k&label=Profile%20Views&color=6a0dad&style=flat-square"/>
<img src="https://img.shields.io/github/followers/rashmini-k?label=Followers&style=flat-square&color=4b0082&labelColor=1a0033"/>
<img src="https://img.shields.io/github/stars/rashmini-k?label=Stars&style=flat-square&color=8a2be2&labelColor=1a0033"/>

</div>

---

## 🟣 About Me

```yaml
name: "Rashmini Kavindya"
role: "Full Stack Developer & UI/UX Designer"
education: "BSc (Hons) Information & Communication Technology — SLIIT (3rd Year)"
prior_qualification: "Higher National Diploma (HND) in Information Technology — Completed"
focus: ["Full Stack Web Development", "UI/UX Design", "User-Centered Engineering"]
traits: ["Fast Learner", "Team Leader", "Detail-Oriented", "Adaptable"]
```

I'm a detail-oriented Information Technology undergraduate with hands-on experience turning real-world requirements into full stack web and mobile applications — from secure authentication systems to e-commerce and IoT-adjacent service platforms. I care about clean architecture, thoughtful UI/UX, and shipping products that are genuinely usable, not just functional.

**🎯 Open To:**
- Full Stack Developer Internships / Junior Roles
- UI/UX Design Collaboration
- Open Source Contributions
- Freelance MERN / Spring Boot Projects

---

## 🟣 Tech Stack

**Languages**

<img src="https://skillicons.dev/icons?i=java,js,kotlin&theme=dark"/>

**Frontend Development**

<img src="https://skillicons.dev/icons?i=react,tailwind,html,bootstrap&theme=dark"/>

**Backend & Databases**

<img src="https://skillicons.dev/icons?i=nodejs,express,spring,mongodb,mysql&theme=dark"/>

**Tools, Design & DevOps**

<img src="https://skillicons.dev/icons?i=git,github,postman,vscode,androidstudio,figma,docker,vercel&theme=dark"/>

---

## 🟣 Core Competencies

| Domain | Focus Area | Details |
|---|---|---|
| Full Stack Development | MERN Stack | React.js, Node.js, Express.js, MongoDB — end-to-end app delivery |
| Enterprise Backend | Java / Spring Boot | Spring Boot, Spring Security, Microservices architecture |
| Mobile Development | Kotlin / Android Studio | Native Android apps with local persistence & scheduled notifications |
| UI/UX Design | Figma | Wireframing, prototyping, and user-centered interface design |
| DevOps Basics | Docker, CI/CD | Dockerized microservices, deployments via Vercel & Render |

---

## 🟣 Featured Projects

<details>
<summary><b>🏥 Care Connect — Remote Caregiving Platform (In Development)</b></summary>
<br/>

Designing a remote caregiving solution enabling real-time monitoring of elderly patients, with integrated secure payments and live communication.

| Aspect | Details |
|---|---|
| **Stack** | MERN Stack, Docker, Stripe, Socket.io |
| **Architecture** | Dockerized microservices |
| **Payments** | Stripe payment gateway integration |
| **Real-Time** | Socket.io for live tracking / communication |
| **Deployment** | CI/CD pipelines on Vercel & Render |
| **Status** | 🚧 Actively in development |

Currently implementing the core authentication system and payment integration while building out the microservices architecture and CI/CD pipeline.

</details>

<details>
<summary><b>🐾 Pet Care System — Central Pet Care</b></summary>
<br/>

A full stack pet services platform combining adoption listings, service bookings, and an e-commerce module, deployed with live demos.

| Aspect | Details |
|---|---|
| **Stack** | MongoDB, Express.js, React.js, Node.js, Tailwind CSS, Docker |
| **Features** | Pet Adoption, Service Booking, E-commerce System |
| **Deployment** | Live demos on Vercel (frontend) & Render (backend) |
| **Academic Context** | 2nd Year – 2nd Semester Project |

</details>

<details>
<summary><b>🏨 Hospitality Management System</b></summary>
<br/>

An internal management system for hospitality operations with real-time booking and role-based dashboards.

| Aspect | Details |
|---|---|
| **Stack** | Java, MySQL, Tailwind CSS |
| **Features** | Real-time appointment booking, Staff/Admin Dashboard |
| **Type** | Internal enterprise-style system |
| **Academic Context** | 2nd Year – 1st Semester Project |

</details>

<details>
<summary><b>🪪 Identity Issuing Service</b></summary>
<br/>

A secure identity issuance platform with authentication and role-based access control.

| Aspect | Details |
|---|---|
| **Stack** | HTML, CSS, JavaScript, PHP, MySQL |
| **Security** | Secure authentication, role-based access (Admin/Staff) |
| **Academic Context** | 1st Year – 2nd Semester Project |

</details>

<details>
<summary><b>📱 HealthyMe — Habit Tracker (Android)</b></summary>
<br/>

A native Android habit-tracking app with scheduled reminders for daily routine building.

| Aspect | Details |
|---|---|
| **Stack** | Kotlin, Android Studio |
| **Features** | Daily habit tracker, scheduled reminders via Shared Preferences |
| **Academic Context** | 2nd Year – 2nd Semester Project |

</details>

---

## 🟣 Learning & Professional Development

| Program | Provider | Status | Focus |
|---|---|---|---|
| Full Stack Web Development (MERN) | SKYREK (Pvt) Ltd. | ✅ Completed | MongoDB, Express, React, Node.js |
| Full Stack Web Development | Springboot Academy (Pvt) Ltd. | 🔄 Ongoing | Java, Spring Boot, Spring Security, Microservices, Angular, TypeScript |
| Higher National Diploma in IT | SLIIT | ✅ Completed (2023–2025) | Core IT & Software Engineering foundations |

---

## 🟣 GitHub Analytics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=rashmini-k&show_icons=true&theme=radical&hide_border=true&bg_color=1a0033&title_color=a78bfa&icon_color=8a2be2&text_color=e0d4ff" width="49%"/>
<img src="https://streak-stats.demolab.com?user=rashmini-k&theme=radical&hide_border=true&background=1a0033&stroke=8a2be2&ring=a78bfa&fire=a78bfa&currStreakLabel=e0d4ff" width="49%"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=rashmini-k&layout=compact&theme=radical&hide_border=true&bg_color=1a0033&title_color=a78bfa&text_color=e0d4ff" width="49%"/>

</div>

---

## 🟣 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=rashmini-k&theme=radical&no-frame=true&column=4&margin-w=15&margin-h=15"/>

</div>

---

## 🟣 Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=rashmini-k&theme=redical&bg_color=1a0033&color=a78bfa&line=8a2be2&point=e0d4ff&hide_border=true" width="100%"/>

</div>

---

## 🟣 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/rashmini-k/rashmini-k/output/github-contribution-grid-snake-dark.svg" width="100%"/>

</div>

> Generated via the [Platane/snk](https://github.com/Platane/snk) GitHub Action — add the workflow to your profile repo to activate.

---

## 🟣 Current Focus

```yaml
learning:
  - "Spring Boot & Spring Security (Microservices Architecture)"
  - "Angular & TypeScript"
  - "Docker-based deployment workflows"
building:
  - "Care Connect — real-time remote caregiving platform"
exploring:
  - "CI/CD pipelines on Vercel & Render"
  - "Advanced UI/UX prototyping in Figma"
open_to:
  - "Full Stack Developer Internships"
  - "Junior Software Engineer roles"
  - "UI/UX collaboration & freelance projects"
```

---

## 🟣 Connect With Me

<div align="center">

<a href="mailto:rashminik727@gmail.com"><img src="https://img.shields.io/badge/Gmail-rashminik727%40gmail.com-6a0dad?style=for-the-badge&logo=gmail&logoColor=white&labelColor=1a0033"/></a>
<a href="https://www.linkedin.com/in/rashmini-k"><img src="https://img.shields.io/badge/LinkedIn-rashmini--k-4b0082?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=1a0033"/></a>
<a href="https://github.com/rashmini-k"><img src="https://img.shields.io/badge/GitHub-rashmini--k-8a2be2?style=for-the-badge&logo=github&logoColor=white&labelColor=1a0033"/></a>
<a href="https://rashmini-kavindya.vercel.app"><img src="https://img.shields.io/badge/Portfolio-View%20Work-a78bfa?style=for-the-badge&logo=vercel&logoColor=white&labelColor=1a0033"/></a>

</div>

---

<div align="center">

*"Good design is invisible — good code is quiet. I aim to build both."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:6a0dad,50:4b0082,100:1a0033&height=120&section=footer" width="100%"/>

</div>
